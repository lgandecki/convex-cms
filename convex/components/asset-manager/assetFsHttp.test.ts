...
describe("getVersionForServing (HTTP file serving logic)", () => {
  describe("access control - only published versions are served", () => {
    it("returns null for draft versions (not yet published)", async () => {
      ...
      const result = await t.query(internal.assetFsHttp.getVersionForServing, {
        versionId,
      });
...
describe("httpServeVersion (action wrapper)", () => {
  it("delegates to getVersionForServing query and returns same result", async () => {
    const t = convexTest(schema, modules);
    ...
    // Call the action (which internally calls the query)
    const actionResult = await t.action(internal.assetFsHttp.httpServeVersion, {
      versionId,
    });

    // Should return the same result as the query
    const queryResult = await t.query(
      internal.assetFsHttp.getVersionForServing,
      { versionId },
    );

    expect(actionResult).toEqual(queryResult);
  });
});
