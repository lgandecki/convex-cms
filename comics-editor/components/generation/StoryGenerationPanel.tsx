...
export function StoryGenerationPanel({
  ...
  const handleGenerate = async () => {
    ...
    try {
      const id = await startGeneration({
        scenarioPath: `comics/stories/${storySlug}/scenarios/${selectedScenario}.json`,
        storySlug,
      });
      setSubmissionId(id);
  ...
  const handleUseVersion = async () => {
    ...
    try {
      ...
      // 5. Commit to asset manager with story-scoped path
      await commitUpload({
        folderPath: `comics/stories/${storySlug}/strips/${selectedScenario}`,
        basename,
        storageId,
        publish: true,
      ...
      setSubmissionId(null);
      router.push(
        `/stories/${storySlug}/scenarios/${encodeURIComponent(selectedScenario)}`
      );
    } catch (error) {
