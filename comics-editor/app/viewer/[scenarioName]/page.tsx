...
export default function ViewerPage({
  ...
  params: Promise<{ scenarioName: string }>;
}) {
  const { scenarioName } = use(params);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "ArrowLeft" && prevStrip) {
        router.push(`/viewer/${encodeURIComponent(prevStrip.scenarioName)}`);
      } else if (e.key === "ArrowRight" && nextStrip) {
        router.push(`/viewer/${encodeURIComponent(nextStrip.scenarioName)}`);
  ...
  }, [router, prevStrip, nextStrip, handleClose]);
  ...
  return (
    <div className="fixed inset-0 bg-black flex items-center justify-center">
      ...
      {prevStrip ? (
        <Link
          href={`/viewer/${encodeURIComponent(prevStrip.scenarioName)}`}
          prefetch={true}
      ...
      {nextStrip ? (
        <Link
          href={`/viewer/${encodeURIComponent(nextStrip.scenarioName)}`}
          prefetch={true}
